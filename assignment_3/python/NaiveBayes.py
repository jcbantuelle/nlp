...
class NaiveBayes:
  ...
  def klass_probability(self, klass, words):
	probability = 0
	for word in words:
		numerator = 1
		if word in self.klasses[klass]:
			numerator += self.klasses[klass][word]
		probability += math.log(numerator / (self.word_count[klass] + len(self.vocabulary)))
	return probability
  ...
  def classify(self, words):
	...
	"""
	pos_probability = self.klass_probability('pos', words) + math.log(self.priors['pos'])
	neg_probability = self.klass_probability('neg', words) + math.log(self.priors['neg'])
  ...
  def addExample(self, klass, words):
	...
	for word in words:
		...
		if word in self.klasses[klass]:
			self.klasses[klass][word] += 1.0
		else:
			self.klasses[klass][word] = 1.0
		
		self.word_count[klass] += 1.0
